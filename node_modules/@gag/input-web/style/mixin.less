@import "~@gag/style-web/themes/default";
@import "~@gag/style-web/mixins/index";
...
.active(@color: @outline-color) {
   border-color: ~`colorPalette("@{color}", 4)`;
   outline: 0;
   border-width: 2px;
   background-image: linear-gradient("@{color}", "@{color}"), linear-gradient(@normal-color,@normal-color);
...
.input() {
  ...
  background-image: linear-gradient(@outline-color, @outline-color), linear-gradient(@normal-color, @normal-color);
  background-repeat: no-repeat;
  background-position: center bottom, center calc(100% - 1px);
